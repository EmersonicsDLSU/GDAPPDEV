...
public class rAmmoDrop : MonoBehaviour
{
    //Item component
    GameObject itemGO;
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        {
            //Debug.LogError("Red ammo collected!");
    ...
    public void collectEffect()
    {
        ...
        GameObject.FindGameObjectWithTag("Gun").transform.GetChild(2).gameObject.SetActive(true);
        RedLaserScript redGun = GameObject.FindGameObjectWithTag("Gun").transform.GetChild(2).gameObject.GetComponent<RedLaserScript>();
        redGun.storedAmmoCount += ammoAdded;
        if (playerStats.currentGun != PlayerStatistics.sRedGun)
        {
            GameObject.FindGameObjectWithTag("Gun").transform.GetChild(2).gameObject.SetActive(false);
        }
    }
