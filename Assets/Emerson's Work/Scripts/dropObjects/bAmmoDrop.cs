...
public class bAmmoDrop : MonoBehaviour
{
    //Item component
    GameObject itemGO;
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        {
            //Debug.LogError("Blue ammo collected!");
    ...
    public void collectEffect()
    {
        ...
        GameObject.FindGameObjectWithTag("Gun").transform.GetChild(0).gameObject.SetActive(true);
        IceShotgunScript blueGun = GameObject.FindGameObjectWithTag("Gun").transform.GetChild(0).gameObject.GetComponent<IceShotgunScript>();
        blueGun.storedAmmoCount += ammoAdded;
        if (playerStats.currentGun != PlayerStatistics.sBlueGun)
        {
            GameObject.FindGameObjectWithTag("Gun").transform.GetChild(0).gameObject.SetActive(false);
        }
    }
