...
public class GreenBulletScript : MonoBehaviour, IGunModel, IBullet
{
    /*DECLARATION START*/

    //time it takes before the bullet got destroyed
    private float bulletDuration = 5.0f;
    private GreenadeLauncherScript greenGun;
    ...
    public void bulletDirection()
    {
        ...
        bulletRb.AddForce(firePoint.right * greenGun.bulletSpeed, ForceMode2D.Impulse);
    }

    //spawns the bullet into its selected location
    public void createBullet(ref GameObject bullet)
    {
        Debug.Log("green bullet spawn");
        //get gun
        ...
        float dirX = greenGun.dirX;
        float dirY = greenGun.dirY;
